...
public class AnalisadorSintaticoGeradorArvore {
	...
	private No ListComan() {
		...
		if(token.getClasse().equals("ID")
				|| token.getImagem().equals("seEsseTrem")
				//Arrumei aqui estava equanto ou invés de enquanto
				|| token.getImagem().equals("no")
				|| token.getImagem().equals("volta")
				|| token.getImagem().equals("bota")
				|| token.getImagem().equals("amostra")
	...
	private No Coman() {
		...
		if(token.getClasse().equals("ID")) {
			...
			no.addFilho(Se());
		else if(token.getImagem().equals("no"))
			no.addFilho(Laco());
	...
	private No Tipo() {
		...
		if(token.getClasse().equals("PR")){
			if(token.getImagem().equals("inteiro") ||
				token.getImagem().equals("realistico") ||
				token.getImagem().equals("prosa") ||
				token.getImagem().equals("vacuo")) {
			
				no.addFilho(new No(token));
				leToken();
			}else {
				Token lastToken = lastToken();
				erros.add("Erro: esperado uma das seguintes palavras reservadas 'inteiro', 'real', 'prosa' ou 'vacuo. Linha: " + lastToken.getLinha() + "Coluna: " + lastToken.getColuna());
			}
